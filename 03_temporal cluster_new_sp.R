...
#new line
exeves_acf <- dummy[, {
  ac <- acf(std_value, lag.max = max_lag, plot = FALSE, na.action = na.pass)$acf
  .(lag = 0:max_lag, acf = as.numeric(ac))}, by = grid_id]

exeves_acf[, lag := rep(1:(1 + max_lag), n_grids)]
# #old
# acf_table <- dcast(exeves_acf, grid_id~lag, value.var = 'V1')
#new
acf_table <- dcast(exeves_acf, grid_id ~ lag, value.var = "acf")
acf_lag_means <- apply(acf_table, 2, mean)[-1]

acf_lag_means <- colMeans(acf_table[, -1], na.rm = TRUE)

evap_acf <- data.table(
  lag = 0:max_lag,
  acf_lag_means = acf_lag_means,
  q05 = apply(acf_table[, -1], 2, quantile, 0.05, na.rm = TRUE),
  q95 = apply(acf_table[, -1], 2, quantile, 0.95, na.rm = TRUE)
  # # CHANGED
  # q10 = apply(acf_table[, -1], 2, quantile, 0.1, na.rm = TRUE),
  # q90 = apply(acf_table[, -1], 2, quantile, 0.9, na.rm = TRUE)
)
...
evap_acf <- data.table(lag = 0:max_lag, acf_lag_means, 
                       ...
                       q95 = apply(acf_table, 2, quantile, 0.95, na.rm = T)[-1]
                       # q10 = apply(acf_table[, -1], 2, quantile, 0.1, na.rm = TRUE),
                       # q90 = apply(acf_table[, -1], 2, quantile, 0.9, na.rm = TRUE)
                       ) # added , na.rm = T
...
# n_grids <- exeves[, max(grid_id)]
#new
n_grids <- length(unique(exeves$grid_id))
...
exeves_acf <- dummy[, {
  ac <- acf(std_value, lag.max = max_lag, plot = FALSE, na.action = na.pass)$acf
  .(lag = 0:max_lag, acf = as.numeric(ac))
}, by = grid_id]

exeves_acf[, lag := rep(1:(1 + max_lag), n_grids)]
acf_table <- data.table::dcast(exeves_acf, grid_id~lag, value.var = 'acf')
acf_lag_means <- apply(acf_table, 2, mean)[-1]
...
# # CHANGED
# evap_acf[, ar_q90 := apply(ensemble_acf_lag_means, 1, function(x) quantile(x, 0.9))]
# evap_acf[, ar_q10 := apply(ensemble_acf_lag_means, 1, function(x) quantile(x, 0.1))]
...
# India
warm_season_start <- as.Date(paste0(sample_year, "-04-01"))
warm_season_end   <- as.Date(paste0(sample_year, "-09-30"))
cold_season_start <- as.Date(paste0(sample_year, "-10-01"))
cold_season_end   <- as.Date(paste0(sample_year + 1, "-03-31"))
...
gg_sample_warm <- ggplot(data = sample_grid_cell[date >= warm_season_start & date <= warm_season_end]) +
  ...
  geom_text(data = definition_names, aes(x, y, label = text), cex = 2.5, fontface = "bold") +
...
ggsave(paste0(PATH_OUTPUT_FIGURES, "clustering_india_gleam4_new_05_95.pdf"), width = 9, height = 12)
